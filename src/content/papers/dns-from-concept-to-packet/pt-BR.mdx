...
O bloco de cima, com `Server:` e `Address:`, é o **resolvedor** que respondeu, em geral o roteador da rede ou o DNS do provedor. O nome
...
revela que o IP veio do **cache** do resolvedor, e não do servidor dono do domínio — esse é o caso comum, e é a primeira manifestação do
cache que veremos em detalhe adiante. Como `www.github.com` é um apelido, o Windows mostra o nome canônico em `Name:` e o que você digitou
em `Aliases:`.
...
A linha `DNS Servers` traz o resolvedor configurado no adaptador, e ele é o mesmo `192.168.40.1` que apareceu como `Server:` na consulta
...
DNS** antes do primeiro pacote. Repare também que, embora eu tenha digitado `www.github.com`, o `ping` exibe o nome canônico `github.com`, o
mesmo encadeamento apelido → canônico que a primeira consulta revelou.
...
2. **Apelido de host.** Um servidor real costuma ter um nome canônico complicado, como `relay1.east.acme.com`. Para os usuários, definem-se
   apelidos mais simples, como `www.acme.com`. O nome bonito que você digita é, por baixo, um apontamento para o canônico de verdade.
3. **Apelido de servidor de e-mail.** A mesma ideia aplicada ao e-mail. O endereço deve ser simples (`bob@acme.com`) mesmo que o servidor de
   ...
   compartilhem o mesmo apelido `acme.com`.
4. **Distribuição de carga.** Sites movimentados são replicados em vários servidores, cada um com um IP. Um único apelido é associado a um
...
O apelido de host fica claro pedindo diretamente o registro CNAME e, num caso rico, observando a cadeia inteira que uma CDN monta:
...
A expressão `canonical name =` é literalmente o nome canônico do conceito. Agora o caso rico, com o `Resolve-DnsName` do PowerShell:
...
O `www.amazon.com` é só o apelido na ponta de uma corrente: ele aponta para `tp.47cf2c8c9-frontier.amazon.com`, que aponta para
...
vez de `NameHost`. O IP e os TTLs mudam a cada consulta — o que importa é a corrente apelido → canônico chegar ao endereço.

O apelido de servidor de e-mail aparece na consulta MX:
...
Cada `mail exchanger` é o nome canônico de um servidor de e-mail, e o `preference` é a prioridade — quanto menor, mais preferencial. O
servidor de menor preferência aqui é o `gmail-smtp-in.l.google.com`, com preferência 5.

A distribuição de carga fica visível quando se consulta um nome muito replicado duas vezes seguidas:
...
Com mais de um endereço, o rótulo muda de `Address:` (singular) para `Addresses:` (plural). É a distribuição de carga: um único nome
associado a um conjunto de servidores, aqui replicados em IPv6 (os `2001:4998:...`, registros **AAAA**) e IPv4 (os `74.6...` e `98.137...`,
registros **A**). Entre as duas execuções, o conteúdo é o mesmo mas a ordem foi rotacionada. Como o cliente quase sempre usa o primeiro
endereço, clientes distintos acabam em réplicas distintas e a carga se reparte.
...
`ANSWERS`. Esse IP autoritativo, `140.82.114.3`, é até diferente do `140.82.113.4` que o resolvedor local nos deu lá no começo — o GitHub
...
já estaria quente no resolvedor do provedor, e as duas medições sairiam quase iguais.
...
- **CNAME** — `Value` é o nome canônico correspondente ao apelido em `Name`. Ex.: `(acme.com, relay1.east.acme.com, CNAME)`.
- **MX** — `Value` é o nome canônico do servidor de e-mail cujo apelido está em `Name`. Ex.: `(acme.com, mail.east.acme.com, MX)`.

O detalhe elegante é que o **mesmo apelido** `acme.com` leva a destinos diferentes conforme o tipo consultado: pede-se **MX** para achar o
...
e na seção `Answers` aparece o registro A com seu `Time to live`. Repare que o bit `Authoritative` é 0: a resposta veio do resolvedor local,
do cache, não do servidor dono do domínio — o mesmo `Non-authoritative answer:` que o `nslookup` mostrava lá no início.

A resposta a uma consulta MX ilustra para que serve a seção adicional. Na seção de resposta vem o nome canônico do servidor de e-mail, e a
seção adicional já traz o registro A com o IP dele, poupando uma segunda consulta.

<Figure caption="Figura 6 — Consulta MX: a resposta traz o nome canônico do servidor de e-mail e o seu IP de uma só vez, na seção adicional.">
...
sequenceDiagram
    ...
    Note over C: Resultado: nome canônico do servidor de e-mail e o IP, juntos
```
...
Até aqui vimos como os registros são lidos. Eles entram assim. Primeiro registra-se o domínio (por exemplo `acme.com`) numa **entidade
registradora**, uma organização credenciada pela **ICANN** (Internet Corporation for Assigned Names and Numbers) que verifica a unicidade do
nome e cobra uma taxa. Você informa o nome e o IP dos seus servidores autoritativos primário e secundário. A registradora insere, nos
servidores TLD `.com`, um par NS + A para cada um. E você insere, nos seus próprios servidores autoritativos, o registro A do servidor Web
no nome canônico, o CNAME que liga o apelido ao canônico e o MX do servidor de e-mail. Há ainda a opção **UPDATE** do protocolo para
adicionar e remover registros dinamicamente [@RFC2136].

<Figure caption="Figura 7 — Inserção de registros: a registradora insere NS e A no TLD, e o dono do domínio insere A, CNAME e MX no seu autoritativo.">
...
sequenceDiagram
    ...
    participant Rgr as Registradora (credenciada pela ICANN)
    ...
    Reg->>Aut: Inserir o A do servidor Web no nome canônico (relay1.east.acme.com, IP, A)
    Reg->>Aut: Inserir o CNAME do apelido (www.acme.com, relay1.east.acme.com, CNAME)
...
**man-in-the-middle**, em que o atacante intercepta consultas e devolve respostas falsas, e o **envenenamento de cache**, em que ele faz um
...
possíveis. O resolvedor aceita a resposta cujo ID bate com o da pergunta, e um identificador tão curto abre espaço para um atacante tentar
...
O DNSSEC assina criptograficamente os registros. Pedindo a um resolvedor validador (`1.1.1.1`) o registro A com a flag `-DnssecOk`, vem
junto a assinatura:
...
O `DNS server failure` e o `RCODE_SERVER_FAILURE` são o **SERVFAIL** do DNS: o resolvedor validador tentou conferir a assinatura, ela não
...
<Warning>
  O resultado depende do resolvedor. Contra `1.1.1.1` ou `8.8.8.8`, que validam DNSSEC, ocorre o SERVFAIL. Contra um resolvedor que não
  valida, `dnssec-failed.org` pode resolver normalmente — e isso mesmo é o ensinamento: a proteção só existe se o resolvedor validar.
</Warning>
...
**servidor autoritativo de verdade** (com BIND ou `dnsmasq` num Linux ou no WSL), cadastrando zonas próprias e apontando o resolvedor da
...
**DNS over TLS** (DoT) contra o DNS clássico na porta 53, e completar o ciclo de inserção de registros pelo lado da registradora e pela
opção de **UPDATE** dinâmico [@RFC2136].
