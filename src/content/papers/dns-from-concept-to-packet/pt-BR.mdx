...
Todo acesso a um site é precedido por uma pergunta silenciosa do host: o endereço IP correspondente a um dado nome. Quem responde é o
**DNS** (Domain Name System), o serviço de diretório da Internet. Este artigo examina essa caixa-preta. A teoria segue os fundamentos de
Kurose e Ross [@KuroseRoss2021], e cada conceito é acompanhado pelo comando que o torna observável, com a saída **real** capturada num
Windows usando `nslookup`, o PowerShell e o Wireshark. As saídas foram coletadas em 6 de junho de 2026 e estão reproduzidas tal como saíram
do terminal.
...
servidores espalhados pelo mundo, e um **protocolo de camada de aplicação** que permite a um host consultar esse banco. O protocolo roda
sobre **UDP** (User Datagram Protocol), na porta **53**, e está especificado nos RFCs 1034 e 1035 [@RFC1034; @RFC1035]. Na prática, os
servidores costumam rodar o software **BIND** (Berkeley Internet Name Domain).

Tudo isso pode ser observado de uma vez com um único comando. O `nslookup` recebe um nome, consulta o servidor DNS e exibe a resposta.
...
O bloco de cima, com `Server:` e `Address:`, identifica o **resolver** que respondeu, em geral o roteador da rede ou o DNS do provedor. O
nome aparecer como `UnKnown` não é erro, significa apenas que esse servidor não tem nome reverso configurado. A linha
`Non-authoritative answer:` revela que o IP veio do **cache** do resolver, e não do servidor dono do domínio — esse é o caso comum, e é a
primeira manifestação do cache examinada em detalhe adiante. Como `www.github.com` é um alias, o Windows mostra o canonical name em `Name:`
e o nome consultado em `Aliases:`.

<Note>
  O ponto final em `www.github.com.` é proposital. Sem ele, o Windows acrescenta o sufixo de busca da rede (aqui, `lan`) e consulta
  `www.github.com.lan` primeiro, poluindo o resultado e, mais adiante, a captura do Wireshark. O ponto final sinaliza que o nome está
  completo e que nada deve ser acrescentado.
</Note>

## O cliente DNS mora na sua máquina

O DNS quase nunca aparece sozinho. O **HTTP** (Hypertext Transfer Protocol), o **SMTP** (Simple Mail Transfer Protocol) e outros o invocam
por baixo para traduzir o nome que o usuário digitou. Considere-se o acesso a `www.acme.com/index.html`:

1. A própria máquina do usuário roda o **lado cliente** do DNS. Ele não é um servidor remoto à parte, reside no host.
...
Como essa tradução acontece **antes** de qualquer dado da aplicação trafegar, o DNS adiciona atraso, às vezes substancial. O cache mitiga
isso: o IP procurado quase sempre já está guardado num servidor próximo, o que reduz tanto o atraso médio quanto o tráfego DNS na rede.

O servidor que respondeu na consulta anterior pode ser identificado com `ipconfig /all`. (A saída foi recortada para as linhas relevantes, e
o endereço físico e os identificadores da máquina estão ocultos.)
...
anterior. Esse endereço chegou automaticamente via **DHCP** (Dynamic Host Configuration Protocol), que distribui as configurações de rede. É
o servidor DNS local, que age como proxy das consultas do host e que é examinado mais de perto na seção sobre a hierarquia.

Que outros protocolos invocam o DNS por baixo confirma-se com um `ping`. Ele resolve o nome **antes** de enviar qualquer pacote.
...
DNS** antes do primeiro pacote. Note-se também que, embora se tenha digitado `www.github.com`, o `ping` exibe o canonical name `github.com`,
...
2. **Alias de host.** Um servidor real costuma ter um canonical name complicado, como `relay1.east.acme.com`. Para os usuários, definem-se
   aliases mais simples, como `www.acme.com`. O nome mais simples digitado pelo usuário é, por baixo, um apontamento para o canonical name
   de verdade.
...
O alias de host fica claro consultando-se diretamente o registro CNAME e, num caso mais rico, observando-se a cadeia inteira que uma CDN
monta:
...
A expressão `canonical name =` é literalmente o canonical name do conceito. Segue o caso mais rico, com o `Resolve-DnsName` do PowerShell:
...
O `www.amazon.com` é apenas o alias na ponta de uma corrente: ele aponta para `tp.47cf2c8c9-frontier.amazon.com`, que aponta para
...
O load distribution fica visível quando um nome muito replicado é consultado duas vezes seguidas:
...
Executando o mesmo comando de novo, a ordem muda:
...
<Note>
  ...
  protocolo de transporte fim a fim (o UDP) para carregar suas mensagens. Há, contudo, uma diferença em relação à Web ou ao e-mail — ninguém
  invoca o DNS diretamente. Ele é uma função-núcleo de suporte, acionada por outras aplicações. É uma decisão de projeto deliberada:
  empurrar uma função crítica para a borda da rede, nos hosts, mantém o núcleo simples.
</Note>
...
Considere-se `pc.globex.com` buscando o IP de `www.acme.com`. A consulta vai primeiro ao servidor local `dns.globex.com`, que percorre a
hierarquia por ele:
...
<Figure caption="Figura 1 — Resolução iterativa típica sem cache: só a primeira consulta é recursiva, e o servidor local pergunta a cada nível (raiz, TLD, autoritativo)." zoomable>
...
<Figure caption="Figura 2 — Resolução totalmente recursiva: cada servidor repassa a consulta ao próximo e a resposta sobe a cadeia. O local não itera." zoomable>
...
<Figure caption="Figura 3 — Variante com servidor intermediário: o TLD aponta o intermediário, não o autoritativo, gerando dez mensagens." zoomable>
...
Esse caminho pode ser percorrido manualmente. Em vez de deixar o servidor local iterar, consulta-se um nível de cada vez, sempre sem
recursão (`-norecurse`) e pedindo os servidores de nome (`-type=NS`). O percurso começa por um servidor raiz:
...
A raiz não sabe o IP de `github.com`, então devolve um **referral** para os servidores do TLD `.com`. Note-se as duas formas de linha:
`nameserver =` é um registro **NS** (o servidor do próximo nível), e `internet address =` é o registro **A** de cola (_glue_) com o IP desse
servidor. Seleciona-se um dos IPs do TLD (`192.41.162.30`, do `l.gtld-servers.net`) e consulta-se a ele:
...
o IP de um deles. Esse IP (`205.251.193.165`) é usado no último salto, agora pedindo o registro **A** e ligando a depuração (`-debug`) para
expor as flags:
...
O padrão referral → referral → resposta é exatamente a resolução iterativa das oito mensagens. Comparando os mundos: nos dois primeiros
saltos não havia `auth. answer`, só `nameserver =`, porque eram delegações. No último salto aparece a flag `auth. answer` e o IP vem na
seção `ANSWERS`. Esse IP autoritativo, `140.82.114.3`, é até diferente do `140.82.113.4` que o resolver local devolveu no começo — o GitHub
publica várias réplicas, e qual delas é alcançada depende de qual servidor responde e do que está em cache.

<Warning>
  No Windows, antes de cada resposta útil, o `nslookup` imprime um bloco de ruído: ele tenta descobrir o nome reverso do servidor apontado e
  falha, mostrando linhas `in-addr.arpa ...`, `(root) ...` ou um `rcode = REFUSED`. Esse bloco do topo e o `Server: UnKnown` podem ser
  ignorados — a resposta que importa vem **depois** dele. Os trechos acima já estão recortados nesse ponto.
</Warning>
...
<Figure caption="Figura 4 — Atalhos por cache: uma resposta repetida vem direto do local, pulando a hierarquia. Com só o TLD em cache, apenas a raiz é contornada." zoomable>
...
Para observar o TTL contando, convém escolher um nome com TTL longo, como o apex `globo.com`. Primeiro limpa-se o cache do cliente e
povoa-se com uma consulta (no PowerShell, como administrador):
...
Lê-se então a entrada já presente no cache do cliente:
...
o TTL em segundos, o número que vai cair. Após vinte segundos sem consultar o nome de novo — já que qualquer nova consulta repõe a entrada
—, lê-se outra vez:
...
corta o atraso, o que pode ser medido com o `Measure-Command`: a primeira consulta vai à rede, a segunda vem do cache.
...
— é o ganho que o cache traz: menos atraso e menos tráfego. Usou-se `www.rnp.br`, um nome menos popular, de propósito: um nome muito
visitado já estaria quente no resolver do provedor, e as duas medições sairiam quase iguais.
...
São quatro os tipos que sustentam tudo o que se viu até aqui:
...
O detalhe elegante é que o **mesmo alias** `acme.com` leva a destinos diferentes conforme o tipo consultado: uma consulta **MX** acha o
servidor de e-mail e uma consulta **CNAME** acha o servidor Web. É isso que permite e-mail e Web compartilharem o nome. Os quatro tipos
foram vistos em ação: o A na primeira consulta, o CNAME na cadeia da Amazon, o MX no Gmail e o NS nos referrals da hierarquia.
...
tem, guarda um par que aponta o caminho: um **NS** (domínio → nome do autoritativo) e um **A** que dá o IP desse autoritativo. É exatamente
esse par NS + A de cola que se viu chegar em cada referral da hierarquia — é ele que faz a iteração funcionar.
...
<Figure caption="Figura 5 — Estrutura da mensagem DNS: cabeçalho de 12 bytes mais quatro seções, cujas seções carregam registros de recurso." zoomable>
...
O cabeçalho tem seis campos de 2 bytes: a **identificação** (16 bits), um número copiado da consulta para a resposta para que o cliente case
...
O Wireshark expõe cada um desses campos no pacote real. Gerou-se uma consulta limpa com `ipconfig /flushdns` seguido de
`nslookup www.example.com.` e filtrou-se por `dns`. Os blocos abaixo reproduzem a árvore do dissecador do Wireshark para o par capturado (o
...
Em `User Datagram Protocol` está a confirmação de que o DNS roda sobre UDP, com a porta 53 num dos lados. O cabeçalho traz o
...
0, pois é uma pergunta) e `Recursion desired`. Agora a resposta:
...
e na seção `Answers` aparece o registro A com seu `Time to live`. Note-se que o bit `Authoritative` é 0: a resposta veio do resolver local,
do cache, não do servidor dono do domínio — o mesmo `Non-authoritative answer:` que o `nslookup` mostrou no início.
...
<Figure caption="Figura 6 — Consulta MX: a resposta traz o canonical name do servidor de e-mail e o seu IP de uma só vez, na seção adicional." zoomable>
...
Vale lembrar que nem todo servidor manda os registros A na seção adicional. Quando não vêm, o cliente faz consultas A separadas depois —
isso é normal, não é erro.

Por fim, o Wireshark torna visível a afirmação de que o DNS adiciona atraso **antes** de a aplicação transferir qualquer dado. Abrindo
`github.com` no navegador e filtrando a consulta DNS junto com o primeiro SYN do TCP, a ordem fica clara:
...
Até aqui o foco esteve em como os registros são lidos. Segue o mecanismo pelo qual eles entram. Primeiro registra-se o domínio (por exemplo
`acme.com`) num **registrar**, uma organização credenciada pela **ICANN** (Internet Corporation for Assigned Names and Numbers) que verifica
a unicidade do nome e cobra uma taxa. O dono informa o nome e o IP dos servidores autoritativos primário e secundário. O registrar insere,
nos servidores TLD `.com`, um par NS + A para cada um. O dono insere, nos servidores autoritativos, o registro A do servidor Web no
canonical name, o CNAME que liga o alias ao canonical name e o MX do servidor de e-mail. Há ainda a opção **UPDATE** do protocolo para
adicionar e remover registros dinamicamente [@RFC2136].

<Figure caption="Figura 7 — Inserção de registros: o registrar insere NS e A no TLD, e o dono do domínio insere A, CNAME e MX no seu autoritativo." zoomable>
...
ping aos 13 servidores raiz, teve impacto mínimo por dois motivos já praticados: filtros bloqueando o ICMP, e os servidores locais já terem
os IPs dos TLDs em cache, contornando a raiz. Um **DDoS contra os servidores TLD** é mais eficaz, porque é mais difícil filtrar consultas
DNS legítimas e os TLDs não são contornados tão facilmente — foi o que derrubou a **Dyn** em 2016, via a botnet **Mirai** de cerca de cem
mil dispositivos IoT, tirando do ar Amazon, Twitter, Netflix, GitHub e Spotify por quase um dia [@Antonakakis2017]. Há ainda o
**man-in-the-middle**, em que o atacante intercepta consultas e devolve respostas falsas, e o **cache poisoning**, em que o atacante faz um
...
O DNSSEC assina criptograficamente os registros. Consultando um resolver validador (`1.1.1.1`) pelo registro A com a flag `-DnssecOk`, vem
junto a assinatura:
...
Várias extensões decorrem naturalmente quando o DNS se junta aos próximos assuntos. A montagem de um **servidor autoritativo de verdade**
(com BIND ou `dnsmasq` num Linux ou no WSL), com o cadastro de zonas próprias e o apontamento do resolver da máquina para ele, faria a
inserção de registros acontecer num servidor real. Uma extensão adicional é a captura e comparação de **DNS over HTTPS** (DoH) e **DNS over
TLS** (DoT) contra o DNS clássico na porta 53, bem como a conclusão do ciclo de inserção de registros pelo lado do registrar e pela opção de
**UPDATE** dinâmico [@RFC2136].
